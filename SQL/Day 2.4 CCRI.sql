...
-- Which customer tenure group is most likely to churn?

SELECT 
	CASE 
		WHEN tenure_in_months <=12 THEN '0-12 months'
		WHEN tenure_in_months <=24 THEN '12-24 months'
		WHEN tenure_in_months <=36 THEN '24-36 months'
		WHEN tenure_in_months <=48 THEN '36-48 months'
	ELSE '48+ months'
	END AS tenured_customers,
	COUNT(*) AS total_customers,
	SUM(CASE WHEN customer_status = 'Churned' THEN 1 ELSE 0 END) AS churned_customers
FROM customer_churn
GROUP BY tenured_customers
ORDER BY churned_customers DESC;

-- Insight: Customers with a tenure of 0–12 months account for the highest number of churned customers, 
--suggesting that the first year of the customer lifecycle is a critical period for retention efforts

-- Which churn reasons are causing the greatest revenue loss?
...
--Insight: The highest revenue loss is associated with customers who churned because a competitor made a better offer, 
--Customers leaving due to competitors offering better devices are another major contributor to revenue loss.

-- Which churned customers contributed the most revenue before leaving?
...
WITH customers AS (
	SELECT 
		...
		SUM(CASE WHEN customer_status = 'Churned' THEN 1 ELSE 0 END) AS churned_customers
	FROM customer_churn
	GROUP BY gender, internet_type, contract
)
